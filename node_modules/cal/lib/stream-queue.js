...
objutil.extend(StreamQueue, events.EventEmitter, {
    ...
    _createNewWriteStream: function (streamX) {
        // Create new stream only if we have something to send into it
        if (!streamX._queueWrites && streamX._pendingWrites.length === 0) {
            return;
        }
        ...
        // Join pending writes with backpressure queue
        streamX._queue = streamX._pendingWrites.concat(streamX._queue);
        streamX._pendingWrites = [];
    ...
    _getStream: function (eventArgs) {
        ...
        if (!streamX) {
            ...
            debug('new streamX pool.length=%d poolId=%d options=%j',
                  calStreamPool.length, poolId, options);
        }
        return streamX;
    },
    ...
    _write: function write(eventArgs) {
        ...
        this._domain.run(function () {
            // XXX(indutny): this is rather hacky, but not more than
            // backpressure handling approach and reconnect logic here.
            // Once that is gone, this can go too.
            function onWrite() {
                streamX._pendingWrites.shift();
            }

            if (!streamX._queueWrites) {
                streamX._pendingWrites.push(eventArgs);

                // Write message, paying attention to backpressure
                streamX._queueWrites =
                    !streamX.stream.write(streamX.format(eventArgs), onWrite);

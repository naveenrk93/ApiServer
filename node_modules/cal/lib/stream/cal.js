...
var os = require('os'),
    ...
    CorkStream = require('cork-stream');
...
function Transformer(options) {
    ...
    // if (deployEnv.isDev()) {
    //     this._host = tconf.host || options.host || 'mscal.qa.paypal.com';
    // }
    // else {
        this._host = 'localhost';   //always connect to caldaemon
    // }
    ...
    this._targetStream = options.targetStream;
...
objutil.extend(Transformer, stream.Transform, {
    ...
    _createSocket: function () {
        ...
        var corked = new CorkStream(socket);
        this._socket = corked;

        var onPipe = this._onPipe.bind(this);
        var onUnpipe = this._onUnpipe.bind(this);
        var onError = this._onError.bind(this);
        var onClose = this._onClose.bind(this);
        var onTimeout = this._onTimeout.bind(this);

        corked.on('pipe',    onPipe);
        corked.on('unpipe',  onUnpipe);
        corked.on('error',   onError);
        corked.on('close',   onClose);
        corked.on('timeout', onTimeout);

        this._cleanupSocket = function _cleanupSocket() {
          corked.removeListener('pipe',    onPipe);
          corked.removeListener('unpipe',  onUnpipe);
          corked.removeListener('error',   onError);
          corked.removeListener('close',   onClose);
          corked.removeListener('timeout', onTimeout);
        };
    ...
    _destroySocket: function () {
        var corked = this._socket;
        if (corked) {
            this._cleanupSocket();
            this._cleanupSocket = null;
            corked.socket.destroy();
        }
    ...
    _connect: function (callback) {
        ...
        if (this._targetStream) {
            ...
            debug('use target stream');
            this._socket = this._targetStream;
            corked = this._socket;
            onConnect();
            return;
        }

        corked = this._createSocket();

        this._state = Transformer.State.CONNECTING;
        debug('socket connect', this._host, this._port);
        corked.socket.connect(this._port, this._host, onConnect);
    },
    ...
    _transform: function (chunk, encoding, callback) {
        ...
        if (this._state === Transformer.State.DISCONNECT) {
            ...
            this._connect(write);
        } else if (this._state === Transformer.State.CONNECTING) {
            this.once('connect', write);
        } else {
            write();
        }
    },

    _gracefulEnd: function () {
        if (this._socket) {

...
var apputil = require('./lib/util/apputil'),
    ...
    cacheLoader = require('./lib/cacheLoader'),
...
module.exports = {
    ...
    init : function (config, confStore) {
        var data, layout, testResult, cached;

        config = config || {};

        if (deployEnv.isDev() || deployEnv.isTest()) {
            console.time('loadingTopos');
            if(process.env.CACHE_TOPOS != 'false') { // jshint ignore:line
              // look for a toposcache.json file
              data = cacheLoader.get();
              cached = true;
            }
            if(!data) {
        ...
        if(process.env.CACHE_TOPOS != 'false' && !!cached) { // jshint ignore:line
          console.log('caching toposcache.json for use of the next run');
          cacheLoader.save(data);
        }
        return data;
    },

...
function createPackage(vault) {
    ...
    return {
        ...
        /**
         * return all the strings with '_algo' suffix
         * @returns {*}
         */
         getAlgoAll: function () {

            if(Object.keys(algoVals).length !== 0) {
                return algoVals;
            }

            for (var algoKey in vault._raw) {
                if (/_algo$/.test(algoKey)) {
                    algoVals[algoKey] = this.get(algoKey);
                }
            }
            return algoVals;
         },

/*
 ...
 DER SEQUENCE DECODER
 THIS UTILITY DECODES THE ANS1 STRUCTURE OF THE GIVEN DER VERSION OF PRIVATE KEY AND
 GIVES OUT THE HEXLIFIED OCTET OF THE PRIVATE KEY (TESTED WITH PKCS#1 ENCODED PRIVATE KEY.
 THE OUTPUT NEEDS TO BE BASE64 ENCODED AND MODIFIED TO PEM FOR PROPER USAGE. PEM IMPL IS
 PRESENT IN THE UTILS CLASS OF THIS MODULE FOR BOTH PRIVATE KEY AND PUBLIC KEY.

 WE USE THE base64-paypal module itself to perform any additional base64 encoding
...
function decode(src){
    if ( src.length % 2 != 0 ){
        console.log('Illegal length. Hex string\'s length must be even.');
    }
    return readASN1(src);
}
    ...
    while ( point < data.length ){
        ...
        if ( tag != 0x5){   // Ignore NULL
            ...
            if ( len > data.length - point ){
                error_message = "LENGTH is longer than the rest.\n";
